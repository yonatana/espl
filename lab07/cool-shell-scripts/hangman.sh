...
wordlib="long-words.txt"						 #there is no types-everything is strings
...
while [ "$guess" != "quit" ] ; do
  ...
  if [ $games -gt 0 ] ; then
    ...
    echo "*** New Game! ***"
									...
									# ""- evrtu thing in between is one argument	''-name of variable 
  games="$(( $games + 1 ))"						#to do a math operation: &(())
